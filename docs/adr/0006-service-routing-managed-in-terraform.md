...
We deliberately avoided `caddy-docker-proxy` (the obvious homelab default). Because Caddy runs on the external RPi4 while services run on separate LXC Docker hosts, label-based discovery would require exposing each host's Docker socket over the network — an unacceptable attack surface across six hosts. Managing routes in Terraform keeps one consistent IaC story with the DNS records and avoids socket exposure.
